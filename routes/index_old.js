...
router.get('/typhoon_line', function(req, res) {  
  (async () => {
    //query = "SELECT * FROM typ_lines.aaline_abby_1983;"
    ...
    var result = await client.query(query);
      ...
      //console.log(result)
      tmpc=FeatureCollection(trackdatac,name_typ);
      //const dataGeoJSONd = GeoJSON.parse(tmpc, { Point: ["lat", "lng"] });
...
const data = []
/*
function FeatureCollection(trackdata){
    //console.log(trackdata);
    trackdata.forEach(function(el){
      console.log(el);
      data.push({lat:el.ycoord,lng:el.xcoord})
    });
    return data;
} */
function FeatureCollection(trackdata,name_typ){
    trackdata.forEach(function(el){
      //console.log(el.st_astext);
  ...
  const data = []
    ...
    if (shape=='point'){
        if (type=="current"){
           ...
           if (type=="current"){
            const query1 = {
                text: 'SELECT * FROM typ_sum.typ_p2 ORDER BY id DESC limit 1'
            }
            function process(){
                let output;
                client.query(query1, (err, result1e) => {
...
router.get('/typhoon/:shape/:type/:name', function(req, res, next) {
	const data = []
    var shape=req.params.shape;
    var type=req.params.type;
    var name_ty = req.params.name;
...
router.get('/datatyphoon/:name/:year', function(req, res, next) {
  var nameTy=req.params.name;
  var yearTy=req.params.year;  
  const querydata = {
    text: 'SELECT * FROM typ_sum.typhoon_sum WHERE name = $1 AND year = $2',
    values: [nameTy,yearTy],
  }
  client.query(querydata, (err, resultd) => {
    if (err) {
      console.log(err.stack)
    } else {
      typhoonsumd = resultd.rows;
      console.log(typhoonsumd);
      res.send(typhoonsumd);     
    }   
  })
})
